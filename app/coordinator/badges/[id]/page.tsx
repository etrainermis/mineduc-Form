...
export default function BadgePreviewPage() {
  ...
  const downloadUsingCanvas = async (elementId: string, filename: string) => {
    ...
    try {
      ...
      console.error("Error in downloadUsingCanvas:", error)
      return false
    }
  }
  ...
  const handleDownloadFallback = () => {
    ...
    try {
      ...
      toast.dismiss(toastId)
      console.log(error);
      toast.error("An error occurred. Please try taking a screenshot manually.")
    } finally {
      setIsDownloading(false)
    }
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Preview and generate badge for {delegate.firstName} {delegate.lastName}
              </p>
            </div>
          </div>
          <UserProfile />
        ...
        <div className="container mx-auto px-4 pb-12">
          <Card className="p-8 bg-white shadow-md">
            <div className="space-y-12">
              ...
              <div id="combined-badge" ref={badgeRef} className="grid gap-12 mx-auto md:grid-cols-2 max-w-[1000px]">
                {/* Front of Badge */}
                <div
                  id="front-badge"
                  ref={frontBadgeRef}
                ...
                >
                  ...
                  <div className="relative flex h-full flex-col items-center py-4">
                    <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                      <Image src="/logo.svg" alt="FutureSkills Logo" width={160} height={45} className="mb-2" />

                      <div className="relative h-28 w-28 overflow-hidden rounded-full border-4 border-[#026FB4] shadow-md">
                        {profileImage ? (
                          // Use the preloaded image with Image component
                      ...
                      <div className="text-center space-y-1">
                        ...
                        <p className="text-base text-gray-600 font-medium">{delegate.position}</p>
                        <p className="text-base text-gray-600">{delegate.organization}</p>
                      </div>
                    </div>

                    <div className="mt-auto">
                      <div className="rounded-xl border-2 border-[#026FB4] bg-white p-2 shadow-md">
                        <QRCode value={`https://forum.com/badge/${delegate.id}`} size={100} className="rounded-lg" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="relative flex h-full flex-col">
                    <div className="mb-4 rounded-xl bg-gradient-to-r from-[#026FB4]/10 via-[#026FB4]/20 to-[#026FB4]/10 p-3 text-center shadow-sm">
                      <p className="text-base font-semibold text-[#026FB4]">Delegate ID</p>
                      <p className="mt-0.5 font-mono text-lg font-bold tracking-wider text-[#026FB4]">
                        {generateRegistrationId(delegate.id)}
                      </p>
                    </div>

                    <h4 className="mb-4 text-center text-lg font-semibold text-[#026FB4]">Your Workshops</h4>
                    ...
                    <div className="flex-1 space-y-3 overflow-y-auto">
                      {workshops.map((workshop) => {
                        ...
                        return (
                          ...
                          >
                            <p className="text-sm font-bold text-gray-800">{workshop.title}</p>
                            <div className="mt-2 flex flex-col">
                              <div className="flex items-center gap-1">
                                <span className="text-xs font-medium text-[#026FB4]">Venue:</span>
                                <span className="text-xs text-gray-700">{workshop.venue}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <span className="text-xs font-medium text-[#026FB4]">Date:</span>
                                <span className="text-xs text-gray-700">{date}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <span className="text-xs font-medium text-[#026FB4]">Time:</span>
