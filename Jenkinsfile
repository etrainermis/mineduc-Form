pipeline {
  ...
  environment {
    ...
    DOCKER_IMAGE = 'etrainermis/forum-frontend'
    DOCKER_CONTAINER='forum-frontend'
    GITHUB_TOKEN = credentials('github-token-id')
    REPO = 'etrainermis/forum-frontend'
  }
    ...
    stage('Checkout') {
      steps {
        checkout([$class: 'GitSCM', branches: [
          ...
          userRemoteConfigs: [
            [url: 'https://github.com/etrainermis/forum-frontend.git', credentialsId: 'etrainer_org_secret']
          ]
        ])
      }
    }
     stage('Set up env variables') {
            steps {
                withCredentials([file(credentialsId: 'fsf_fe_env', variable: 'FSF_ENV_FE_FILE')]) {
                    script {
                        ...
                        cp "${FSF_ENV_FE_FILE}" /tmp/env-files/.env   # Copy the file to a temporary directory
        ...
        script {
          sh """
            sshpass -p '${DEPLOY_PASSWORD}' ssh -o StrictHostKeyChecking=no $DEPLOY_USER@$DEPLOY_HOST -p $DEPLOY_PORT '
              ...
              docker run -d --restart always --name ${DOCKER_CONTAINER} -p 3035:3035  ${DOCKER_IMAGE}:latest
            '
          """
